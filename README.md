...
### [Ironcurtain](https://github.com/provos/ironcurtain) - Plain-English constitution policy for agent runtime security
By Niels Provos (Google Security). Write your security intent in plain English — the system compiles it into deterministic rules enforced on every tool call. Two modes: V8 isolate (builtin agent) or Docker agent mode with TLS-terminating MITM proxy. Every MCP tool call passes through allow/deny/escalate policy. The agent is always untrusted. 445 stars, Apache 2.0, TypeScript. Different from RAMPART (pytest adversarial tests), nono (capability brokering), or Greywall (kernel primitives) — Ironcurtain compiles natural language policy into executable enforcement. The constitution approach means non-security-experts can define agent boundaries.

### [iFixAi](https://github.com/ifixai-ai/iFixAi) - Behavioral misalignment diagnostic for AI agents
32 tests across five pillars: fabrication, manipulation, deception, unpredictability, opacity. Provider-agnostic (OpenAI, Anthropic, Bedrock, Azure, Gemini). Mandatory minimums: tool authorization must score 100%, privilege escalation resistance 95%. Letter grade in under 5 minutes. Content-addressed manifest for bit-identical replay. 423 stars, Apache 2.0, Python. Where RAMPART tests adversarial attack resistance and ExploitBench tests exploit chains, iFixAi tests for misalignment patterns — the gap between what you asked the agent to do and what it actually does. Useful for comparing LLM safety across providers before committing to one in production.
